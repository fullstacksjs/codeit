...
describe('prismaPuzzleRepo', () => {
  ...
  it(
    ...
    pipe(
      TO.tryCatch(() =>
        client.puzzle.findFirst({ where: { title: sample?.title } }),
      ),
      TO.chain(TO.fromNullable),
      TO.chain(({ id }) => puzzleRepo.getPuzzleById(id as Id)),
      TO.match(fail, puzzle =>
        expect(puzzle.statement).toEqual(sample?.statement),
      ),
    ),
  );
  it(
    ...
    pipe(
      ...
      TO.match(fail, puzzle =>
        expect(puzzle.inputDescription).toEqual(sample?.inputDescription),
      ),
    ),
  );
